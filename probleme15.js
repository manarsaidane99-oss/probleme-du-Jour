...
```*/
function mettreEnMajuscules(tableauChaines) {
return tableauChaines.map(chaine => {return chaine.toUpperCase()})
}


console.log(mettreEnMajuscules(['bonjour', 'monde', 'tourne', 'pois']));
...
function paysLesPlusPeuples(tableauPays) {
  return tableauPays.filter((pays)=>{return pays.population>500000000})
}
...
let tableauNombres = [2, 6, 20, 8, 14];


function diviserParDeux(tableauNombres) {
  return tableauNombres.map((nombre)=>{return nombre/2})
}
console.log(diviserParDeux(tableauNombres))

 


/*function valeurs(objet) {
  return objet.reduce((t,element)=>{
     t.push(objet[element])
    return t},[])
}
console.log(valeurs({
    premier: 5,
 ...
 function chaineLaPlusCourte(tableauChaines) {
  return tableauChaines.reduce((chCourte,chaine)=>{
    if(chCourte.length>chaine.length){
      return chaine
    }
    else{
       return chCourte
...
console.log(chaineLaPlusCourte(["manar","mana","man","m"]))